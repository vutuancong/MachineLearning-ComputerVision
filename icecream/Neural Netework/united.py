...
def load_train_data(split_percent = 0.9):
	...
	shuffed_feature_data = np.array(feature_data)[shuffle_indices]
	shuffed_label_data = np.array(label_data)[shuffle_indices]
	split_index = int(split_percent * len(shuffed_feature_data))
	
	feature_train = shuffed_feature_data[:split_index]
	feature_test = shuffed_feature_data[split_index:]
	label_train = shuffed_label_data[:split_index]
	label_test = shuffed_label_data[split_index:]
